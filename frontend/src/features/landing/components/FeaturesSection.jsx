...
export default function FeaturesSection() {
  ...
  return (
    <section id="features" ref={containerRef} className="py-24 px-6 bg-[#FFFFFF]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, i) => (
            <div
              key={i}
              className="feature-card opacity-0 bg-white border border-[#bc1313]/20 rounded-2xl p-8 hover:border-[#bc1313] transition-colors shadow-xl"
            >
              <div className="bg-[#bc1313]/10 w-12 h-12 rounded-lg flex items-center justify-center mb-6">
                {feature.icon}
              </div>
              <h3 className="text-lg font-medium text-[#430202] mb-4">{feature.title}</h3>
              <p className="text-sm text-[#430202]/80 leading-relaxed">
                {feature.description}
              </p>
            </div>
          ))}
