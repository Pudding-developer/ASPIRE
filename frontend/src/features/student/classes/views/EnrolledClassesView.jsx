...
import { Search, Plus, BookOpen, ArrowRight, Loader2, Calendar, Hash, X, CheckCircle2, AlertCircle } from 'lucide-react';
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          ...
          >
            <button
              onClick={onClose}
              className="absolute right-4 top-4 rounded-md border border-[#f0e8e8] bg-white p-1 text-gray-400 hover:bg-gray-50 hover:text-gray-700"
            >
              <X size={18} />
            </button>
...
function ClassCard({ cls, index, onClickView }) {
  ...
  const daysEnrolled = enrolledDate
    ? Math.max(0, Math.floor((Date.now() - enrolledDate.getTime()) / (1000 * 60 * 60 * 24)))
    : null;
  const enrolledLabel = enrolledDate
    ? enrolledDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
    : 'N/A';
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 pl-2">
        ...
        <h3 className="line-clamp-2 text-[1.9rem] font-extrabold leading-tight text-[#111827] md:text-[2rem]">
          {cls.subject_name}
        </h3>
      </div>
      ...
      <div className="mb-5 border-t border-gray-100 pt-4 pl-2">
        ...
        <p className="mt-3 text-[12px] font-medium text-gray-500">
          {daysEnrolled === null
            ? 'Enrollment timeline unavailable'
            : daysEnrolled === 0
              ? 'Joined today'
              : `Joined ${daysEnrolled} day${daysEnrolled > 1 ? 's' : ''} ago`}
        </p>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between border-t border-gray-100 pt-4 pl-2">
        ...
        <span className="flex items-center gap-1.5 text-[14px] font-bold text-[#111827] transition-colors" style={{ color: accent }}>
...
export default function EnrolledClassesView({ classes, onRefresh }) {
  ...
  const yearOptions = [...new Set(classList.map((cls) => String(cls.year_level)).filter(Boolean))];
  const semesterOptions = [...new Set(classList.map((cls) => String(cls.semester)).filter(Boolean))];
  ...
  const filtered = classList
    ...
    .sort((a, b) => {
      ...
      const aDate = a.enrolled_at ? new Date(a.enrolled_at).getTime() : 0;
      const bDate = b.enrolled_at ? new Date(b.enrolled_at).getTime() : 0;
      return bDate - aDate;
    });
    ...
    <div className="space-y-6 bg-[radial-gradient(circle_at_top_left,rgba(188,19,19,0.08),transparent_24%),radial-gradient(circle_at_95%_0%,rgba(91,91,232,0.08),transparent_20%)] p-8" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div className="space-y-2">
          ...
          {!loading && classList.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="rounded-full border border-[#eadede] bg-[linear-gradient(180deg,#ffffff_0%,#fff7f7_100%)] px-3 py-1 text-[12px] font-semibold text-gray-700 shadow-sm">
                {classList.length} total classes
              </span>
              <span className="rounded-full border border-[#eadede] bg-[linear-gradient(180deg,#ffffff_0%,#fff7f7_100%)] px-3 py-1 text-[12px] font-semibold text-gray-700 shadow-sm">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-[linear-gradient(180deg,#d11717_0%,#a81010_100%)] px-5 py-2.5 text-[13px] font-bold text-white shadow-[0_10px_22px_rgba(188,19,19,0.25)] transition-all hover:brightness-105"
        >
          <Plus size={15} />
          Join Class
        </button>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="rounded-lg border border-[#e8e6e0] bg-white px-3 py-2 text-[12px] font-semibold text-gray-700 hover:bg-gray-50"
                >
                  Clear
                </button>
              )}
      ...
      {!loading && classList.length > 0 && filtered.length === 0 && (
        <div className="rounded-2xl border border-[#eadede] bg-[linear-gradient(180deg,#ffffff_0%,#fff9f9_100%)] p-10 text-center shadow-sm">
          ...
          <button
            ...
            className="mt-5 rounded-lg border border-[#e8e6e0] px-4 py-2 text-[13px] font-semibold text-gray-700 hover:bg-gray-50"
          >
            Clear filters
          </button>
        </div>
        ...
        >
          {filtered.map((cls, i) => (
            <motion.div key={cls.id} variants={cardVariants}>
              <ClassCard cls={cls} index={i} onClickView={setSelectedClass} />
            </motion.div>
          ))}
        </motion.div>
      )}
