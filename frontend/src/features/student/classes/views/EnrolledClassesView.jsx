  ...
  return (
    <button type="button" onClick={() => onClickView(cls)}
      ...
      <div className="mt-auto flex items-center justify-between border-t border-gray-100 pt-4 pl-2">
        ...
        <span className="flex items-center gap-1.5 text-[14px] font-bold transition-colors" style={{ color: accent }}>View Class <ArrowRight size={16} /></span>
      </div>
    </button>
  );
}

function ArchivedCard({ cls, index }) {
  const accent = ACCENT_COLORS[index % ACCENT_COLORS.length];
  ...
  return (
    <div className="relative flex flex-col overflow-hidden rounded-xl border border-[#e8e8e8] bg-[linear-gradient(180deg,#fafafa_0%,#f5f5f5_100%)] p-5 opacity-80">
      ...
      <div className="mt-auto flex items-center justify-between border-t border-gray-200 pt-4 pl-2">
        <span className="text-[12px] font-semibold text-gray-400">Enrolled: {enrolledLabel}</span>
...
/* ─── Main View ─── */
export default function EnrolledClassesView({ classes, archivedClasses, onRefresh }) {
  ...
  return (
    <div className="space-y-6 bg-[radial-gradient(circle_at_top_left,rgba(188,19,19,0.08),transparent_24%),radial-gradient(circle_at_95%_0%,rgba(91,91,232,0.08),transparent_20%)] p-8" style={{ fontFamily: 'Inter, sans-serif' }}>
        ...
        <>
          ...
          {!loading && filtered.length > 0 && (
            <motion.div variants={gridVariants} initial="hidden" animate="visible" className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
              {filtered.map((cls, i) => (
                <motion.div key={cls.id} variants={cardVariants}><ClassCard cls={cls} index={i} onClickView={setSelectedClass} /></motion.div>
              ))}
            </motion.div>
          )}
        </>
      ...
      <AnimatePresence>
        {selectedClass && <ClassDirectoryModal cls={selectedClass} onClose={() => setSelectedClass(null)} />}
      </AnimatePresence>
    </div>
  );
}
