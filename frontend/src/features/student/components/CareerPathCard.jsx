...
export default function CareerPathCard({ match, index, selected, optimal, onSelect, isChosenGoal, onSetAsGoal, careerLoading }) {
  ...
  return (
    <div
      ...
      className={`w-52 min-w-[208px] min-h-[220px] rounded-2xl p-5 cursor-pointer transition-all border flex-shrink-0 flex flex-col h-full relative overflow-hidden
        ${isChosenGoal
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <span className={`text-[8px] font-bold tracking-[0.1em] uppercase ${selected ? 'text-white/60' : 'text-gray-400'}`}>
          {cat}
        </span>
